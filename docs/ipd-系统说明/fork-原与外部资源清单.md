...
| 资源 | 来源 | 用途 | 引用位置 | 现状 |
|---|---|---|---|---|
| `IPD系统_AI开发主Prompt_v3.md` | 产品 v3 原文 | **唯一权威规格**（1368 行 / 99 KB） | 开发说明书 §1、§3、§5、§8、§11 | ✅ **已填充完整版**（2026-09-04 从 ZK-IPD `产品流程细化管理工具/` 复制，替换之前的 11.5 KB / 181 行简化版） |
| `IPD系统_六阶段标准动作清单_v3.md` | 产品 v3 | 69 动作 seed（深管 42 / 轻管 27 / 阻断 38） | 开发说明书 §5.4 BR-IPD-02 | ✅ **已填充**（24 KB / 298 行） |
| `IPD系统_五大Gate评审要素_v1.md` | 产品 v1 | 33 项要素 + 14 否决项 seed | 开发说明书 §5.6 BR-GATE-01b | ✅ **已填充**（17 KB / 229 行） |
| `IPD系统_验收清单.md` | 产品 | 237 条 AC（P0–P4 归类，实测） | 开发说明书 §11 各阶段验收 | ✅ **已填充**（36 KB / 411 行） |
| `IPD系统_开发执行规则_AI必读.md` | 产品 | 11 条硬约束 + 状态机速查 + 子代理派发模板 | 开发说明书 §3 | ✅ **已填充**（11 KB / 191 行） |
| `IPD系统_冲突裁决与最终待确认清单.md` | 产品 | 4 条裁决原则 + 6 项裁定结论 + V3.1 内部矛盾分析 | 开发说明书 §3、§14.1 | ✅ **已填充**（12 KB / 209 行） |
| `IPD系统_待确认决策表_v2.md` | 产品 | 33 项决策原始记录（按 A-G 七类组织） | 开发说明书 §5、§8 | ✅ **已填充**（30 KB / 172 行） |

### 2.2 设计规范（2 个）

| 资源 | 来源 | 用途 | 引用位置 | 现状 |
|---|---|---|---|---|
| `assets/公共规范-通用.md` | RuoYi-AI 设计规范 | type 骨架（按钮 / 表格 / 表单 token） | `_公共规范.md` §一 | ⚠️ 骨架（前端仓库使用，本仓库不需要） |
| `design-specs/后台-RuoYi-AI.md` | RuoYi-AI 设计规范 | Element Plus 组件表 | `_公共规范.md` §六 | ⚠️ 骨架（前端仓库使用，本仓库不需要） |

### 2.3 演示数据（1 个）

| 资源 | 来源 | 用途 | 引用位置 | 现状 |
|---|---|---|---|---|
| `mock-data.js` | 前端原型 | 演示数据（13 角色 + 3 项目 + 4 算例） | 开发说明书 §13 + _导航地图 §6 | ✅ **已生成**（ZK-IPD 未发现；Claude Code 基于 v3 Prompt + 开发说明书 + 决策表综合生成，~15 KB） |

---

## 三、本仓库提供的外部资源骨架

外部资源现状（2026-09-04 二轮清理后）：`外部资源/` 下 10 个文件**原文已全部填充**（v3 核心 7 件 + 前端规范 2 件 + mock-data.js），骨架阶段已结束；v2 / 早期历史件 5 个已删除（吸收判定：熵基环节已被 v3 清单收录 5 处、决策已全部回填 v3、P0 任务以开发说明书 §11 + 改造检查清单为准），git 历史可恢复。
...
**填充状态（2026-09-04 终局）**：以上资源已全部填充完毕——v3 核心 7 件由 Gavin 原文入库（验收清单 v2.1 = 237 条 AC，实测），mock-data.js 由 Claude Code 按 v3 + 决策表生成（Q2 = 回款口径），assets / design-specs 为前端仓库使用的规范原件。后续口径变更以 v3 Prompt 为唯一事实源，并登记 `log.md`。

---

## 四、各资源填补决策表（历史记录，已于 2026-09-04 全部完成）
