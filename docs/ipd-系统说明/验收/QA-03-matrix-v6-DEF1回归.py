#!/usr/bin/env python3
# QA-03 权限与审计矩阵实测 v6（DEF-1 修复后回归）（真库 16039；基准=初始化真实数据）
...
CASES += [
    ...
    ("POST /cert-templates/999999999/remove","POST","/api/v1/cert-templates/999999999/remove",None,
     {"ADMIN":"not403","LEADER":"403","MARKET":"403","RD":"403","NOAUTH":"401"}),
    ("POST /gate-elements(DEF-1已修)","POST","/api/v1/gate-elements",
     {"gateCode":"G1","elementCode":"Q"+TS,"elementName":"QA03要素v6","passStandard":"标准",
      "isVeto":"0","sortOrder":99,"enabled":"1"},
     {"ADMIN":"200","LEADER":"403","MARKET":"403","RD":"403","NOAUTH":"401"}),
...
aud = {
  ...
  "GATE_ELEMENT_CREATE_本轮JSON合规": sql("SELECT COUNT(*) FROM audit_logs WHERE entity_type='GATE_ELEMENT' AND action='CREATE' AND JSON_VALID(after_data)"),
...
out = {"head":HEAD,"ts":TS,"product_created":FREE,"project_created":NEW_PROJ["id"],
       ...
       "defects":["DEF-1 已修复(b74f46bf)：本轮 ADMIN 建要素 200 + 审计 JSON_VALID=1",
...
with open("/tmp/qa03_result_v6.json","w") as f: json.dump(out,f,ensure_ascii=False,indent=1)
