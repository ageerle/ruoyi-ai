...
| 项 | 内容 | 状态 |
|---|---|---|
| ① | `audit_logs.seq` 去 `AUTO_INCREMENT` | **✅已落地**（owner 指令「立即完整执行」，2026-09-05 21:59:13 PDT 活库执行 `MODIFY COLUMN seq BIGINT NOT NULL`，注释同步基线；停写窗口内完成，详见冒烟 JSON `AUDIT-CHAIN-P变体真库冒烟-20260905.json`） |
| ② | 新增 `audit_log_chain_heads` 单行锚表 + 登记 `tenant.excludes` | **✅已落地**（seed-sync 21:59 执行：GLOBAL last_seq=1626/next_seq=1627/last_hash=尾行 curr_hash，seq_ok=1 hash_ok=1；`tenant.excludes` 登记 `audit_log_chain_heads` 已随 HEAD `21131def` 入库 @L266，部署件内 yml 已验证含该行；基线 DDL 同步补 21b 建表+GLOBAL seed，见下） |
| ③ | `append` 改为 chain_heads 原子递增（**P 悲观锁**，owner 21:30 拍板）→ 去 `insertStrategy=NEVER` → 删除死代码 `catch (DuplicateKeyException)` 整段 | **✅已落地**（Java 三件：`AuditLog` 去 NEVER→`@TableField("seq")`、`AuditChainHead` 实体 + `AuditChainHeadMapper`（selectForUpdate/advance）、`AuditLogService.append` P 变体重写（锚行锁→分配→advance 恒 1 断言→insert，advance=0/锚缺失双 fail-fast）+ 删 selectLast/orderBySeq 死代码；契约测：新 `AuditChainHeadAppendContractTest` 6 例 + Symmetry 2 旧重试用例改 P 语义 + PayloadGuard 锚行 stub + P054 构造器补参；定向 35/35 绿 + 全模块 485/0/0/22 绿（基线 476+9）；部署件 `ruoyi-admin-ch.jar` 起于 16045，真库冒烟 12/12 PASS：单发 seq=1641 锚行分配、并发 4 登录 seq 1642–1645 连续无跳号无冲突、锚行同步 1645/1646、verify chain=OK broken=0 hashBroken=0 gaps=[]、console 0 ERROR 零锁异常） |
...
| ⑦ | **`hash_version` 处置**（原定性：62 行误标 v2 + 实体缺字段 + 一致性测） | **重定性：急性问题已随清库消失**（21:10 现态 207/207 行全 NULL、非 NULL 残留 0；列语义 NULL=legacy-v1 与现行 v1 算法天然一致；`AuditHashChain` 已内置 canonicalV1/V2/byVersion 双版本机制）。残留=休眠列，处置包见蜂群 turnkey 包 §3（**owner 已拍选项 a 维持休眠** 21:30——⑦ 关卡收口，休眠列零改动；日后接线须再立 owner 决策） |
...
- **Q6 执行追记（21:30–21:35，owner 授权后）**：owner 裁决「按照建议执行」（CAS 拍 P + ⑦ 拍 a）+「Q6 REVOKE 授权」→ 本会话按 turnkey §2.2 执行 REVOKE（前置五探针复验全清，0 活跃连接窗口仍开）→ 四向探针全过（负向 1142 拒绝 / chain_heads UPDATE rc=0 / FOR UPDATE rc=0 / 业务表 rc=0）→ **DEF-5 收口**：audit_logs 只追加已在 DB 层强制。上文「零写库」纪律指本会话蜂群起草轮；本次 REVOKE 为 owner 明示授权的唯一 DB 级变更，实录见 turnkey 包 §10。
...
| **Q6** | **DEF-5**（库级 grant 架空表级只追加） | 独立，可先行 | **已交付 REVOKE runbook**（蜂群 turnkey 包 §2）：铁证（库级 S,I,U,D vs audit_logs 表级 S,I）+ 最小 REVOKE（只收库级 UPDATE,DELETE）+ 安全边界全闭合（125 表/124 表级覆盖、0 view/routine/trigger/event、单 host 变体、0 列级权限）+ 三步验证/回滚。**已执行**（21:30 owner 授权 → 21:33 前置五探针复验全清 → 21:34 执行 → 字典双验 + 负向 1142 + 三正向对照全过，**DEF-5 收口**；实录 turnkey 包 §10） |
